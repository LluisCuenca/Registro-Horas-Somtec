...
(async () => {
  ...
  await page.locator("#periodPanel").getByText("Vacaciones").first().waitFor();
  await page.locator("#periodPanel").getByText("Matriz trabajador").waitFor();
  await page.locator("#periodPanel").getByText("Resumen por tarea").waitFor();
  await page.locator("#historyTaskFilter").selectOption({ label: "Servicios" });
  await page.locator("#historyTypeFilter").selectOption({ label: "Horas extras" });
  await page.locator("#periodPanel").getByText("Servicios / Soporte cliente").first().waitFor();
  await page.locator("#historyTaskFilter").selectOption("all");
  await page.locator("#historyTypeFilter").selectOption("all");
  ...
  await page.locator("div#configEmployeePhotoPreview.profile-preview").waitFor();
  await page.locator("#configEmployeePhoto").setInputFiles(path.resolve(__dirname, "..", "assets", "favicon.png"));
  await page.locator("img#configEmployeePhotoPreview.profile-preview").waitFor();
  await page.getByRole("button", { name: "Guardar", exact: true }).click();
